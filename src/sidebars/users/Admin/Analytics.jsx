        ...
        >
          {analyticsData.monthlyPerformance && analyticsData.monthlyPerformance.length > 0 ? (
              ...
              <LineChart
                data={analyticsData.monthlyPerformance.map((m, idx) => ({
                  label:
                    ...
                    `${m.month || ""} ${m.year || ""}`.trim() ||
                    `M${idx + 1}`,
                  achieved: Number(m.achieved || m.achievedValue || 0),
                  target: Number(m.target || m.targetValue || 0),
                }))}
            ...
            <p className="text-xs text-gray-400">
              No monthly history yet. Once the backend returns `monthlyPerformance`, this chart will render.
            </p>
          )}
        </ChartCard>
