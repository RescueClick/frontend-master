...
function presetToRange(preset, customStart, customEnd) {
  ...
  if (preset === "mtd") return { start: toYmd(new Date(now.getFullYear(), now.getMonth(), 1)), end: toYmd(tomorrowStart) };
  if (preset === "qtd") {
    const qStartMonth = Math.floor(now.getMonth() / 3) * 3;
    return { start: toYmd(new Date(now.getFullYear(), qStartMonth, 1)), end: toYmd(tomorrowStart) };
  }
  if (preset === "ytd") return { start: toYmd(new Date(now.getFullYear(), 0, 1)), end: toYmd(tomorrowStart) };
  const s = new Date(todayStart); s.setDate(s.getDate() - 30);
  return { start: toYmd(s), end: toYmd(tomorrowStart) };
}
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: designSystem.colors.background }}>
        ...
        <div className="mb-6">
          <PageHeader
            ...
            right={
              <button
                type="button"
                onClick={() => navigate("/rsm/dashboard")}
      ...
      {/* KPI section moved below Metrics Cards */}
        ...
        {/* Target vs Achievement - RSM (This Month) */}
        <div className={`${designSystem.card.base} ${designSystem.card.padding} mb-6`}>
          ...
          <p className="text-xs text-gray-500 mb-4">
            Disbursed vs assigned target for this month (RSM hierarchy)
          </p>
