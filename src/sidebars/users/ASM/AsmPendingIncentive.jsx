...
const AsmPendingIncentive = () => {
  ...
  // Filter for pending incentives (not yet eligible - must EXCEED targets, not just meet them)
  const pendingIncentives = incentives.filter((i) => {
    // Show only partners who haven't EXCEEDED targets (eligible for incentives)
    // Partners are eligible only if they EXCEEDED targets (not just met them)
    return !i.targetExceeded || !i.targetAchieved || (!i.fileTargetMet || !i.disbursementTargetMet);
  });
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-amber-600 mt-2">{filteredIncentives.length}</p>
          </div>
                  ...
                  <tr key={incentive.partnerId} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-2 py-3 align-middle">
                      <span className="px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                        {incentive.targetAchieved ? "Eligible" : "Pending"}
                      </span>
                    </td>
                  </tr>
                );
